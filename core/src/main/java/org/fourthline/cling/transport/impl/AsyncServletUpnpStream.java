...
public abstract class AsyncServletUpnpStream extends UpnpStream implements AsyncListener {
    ...
    public void run() {
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("Processing new request message: " + requestMessage);
            ...
            if (!getResponse().isCommitted()) {
                log.debug("Response hasn't been committed, returning INTERNAL SERVER ERROR to client");
    ...
    public void onComplete(AsyncEvent event) throws IOException {
        if (log.isDebugEnabled())
            log.debug("Completed asynchronous processing of HTTP request: " + event.getSuppliedRequest());
    ...
    public void onTimeout(AsyncEvent event) throws IOException {
        if (log.isDebugEnabled())
            log.debug("Asynchronous processing of HTTP request timed out: " + event.getSuppliedRequest());
    ...
    public void onError(AsyncEvent event) throws IOException {
        if (log.isDebugEnabled())
            log.debug("Asynchronous processing of HTTP request error: " + event.getThrowable());
    ...
    protected StreamRequestMessage readRequestMessage() throws IOException {
        ...
        if (log.isDebugEnabled())
            log.debug("Processing HTTP request: " + requestMethod + " " + requestURI);
    ...
    protected void writeResponseMessage(StreamResponseMessage responseMessage) throws IOException {
        if (log.isDebugEnabled())
            log.debug("Sending HTTP response status: " + responseMessage.getOperation().getStatusCode());
