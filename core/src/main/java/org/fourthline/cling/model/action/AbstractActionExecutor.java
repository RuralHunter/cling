...
public abstract class AbstractActionExecutor implements ActionExecutor {
    ...
    private static Logger log = LoggerFactory.getLogger(AbstractActionExecutor.class.getName());
    ...
    protected Object readOutputArgumentValues(Action<LocalService> action, Object instance) throws Exception {
        ...
        log.debug("Attempting to retrieve output argument values using accessor: " + results.length);

        int i = 0;
        for (ActionArgument outputArgument : action.getOutputArguments()) {
            log.debug("Calling acccessor method for: " + outputArgument);
